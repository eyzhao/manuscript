...
polarBarPlot <-
  function(
    ...
    df<-plyr::ddply(df, plyr::.(item), plyr::mutate, ymin=c(1,ymax[1:(length(ymax)-1)]))
	...
	fullItems = c(items, paste(substr(df$item, 5, 5), '-', substr(df$item, 7, 7)))
    ...
    dfItemLabels<-plyr::ddply(df, plyr::.(item),summarize,xmin=xmin[1])
...
#      data=dfItemLabels)
    ...
    # family labels
    if(familyLabels){
      #familyLabelsDF<-plyr::ddply(df, plyr::.(family),summarise,x=mean(xmin+binSize),angle=mean(xmin+binSize)*(-360/totalLength)-alphaStart*180/pi)
	...
	colorVector = hsv(h=sort(rep(c(0, 0.3, 0.6, 0.9), 4)), s=rep(c(0.1, 0.2, 0.5, 1), 4), v=rep(c(0.8, 0.8, 0.8, 0.8), 4))
